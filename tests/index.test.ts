...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new DjeliaSDK({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['DJELIA_SDK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new DjeliaSDK({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['DJELIA_SDK_BASE_URL'] = 'https://example.com/from_env';
      const client = new DjeliaSDK({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['DJELIA_SDK_BASE_URL'] = ''; // empty
      const client = new DjeliaSDK({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });

    test('blank env variable', () => {
      process.env['DJELIA_SDK_BASE_URL'] = '  '; // blank
      const client = new DjeliaSDK({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new DjeliaSDK({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['DJELIA_SDK_BASE_URL'] = 'http://localhost:5000/env';
      const client = new DjeliaSDK({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new DjeliaSDK({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new DjeliaSDK({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['DJELIA_SDK_API_KEY'] = 'My API Key';
    const client = new DjeliaSDK();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['DJELIA_SDK_API_KEY'] = 'another My API Key';
    const client = new DjeliaSDK({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new DjeliaSDK({ apiKey: 'My API Key', fetch: testFetch });
